...
def get_login_keyboard():
    ...
    builder.add(
        InlineKeyboardButton(text="Войти с аккаунта spredly.ru", callback_data="login")
    )
    ...
    async with db_helper.session_factory() as session:
        ...
        if not user:
            await message.answer(
                f"Если у Вас нет аккаунта, зарегестрируйтесь на spredly.ru!",
                reply_markup=get_login_keyboard(),
            )
        else:
            await message.answer(
                f"Ваш аккаунт {user.email} привязан к боту. Вы будете получать все актуальные отчеты ИИ!"
            )
...
async def get_password_handler(message: types.Message, state: FSMContext):
    ...
    async with db_helper.session_factory() as session:
        ...
        if not user:
            await message.answer(
                "Неккоретный логин или пароль.", reply_markup=get_login_keyboard()
            )
        ...
        if not user.telegram_id:
            ...
            await message.answer(
                f"Ваш аккаунт {user.email} успешно привязан к этому боту. Вы будете получать все актуальные отчеты ИИ!"
            )
    ...
    async with get_bot() as bot:
        async with db_helper.session_factory() as session:
            ...
            if len(child_ids):
                ...
                child_id = 0
            report = await get_straight_full_history(
            ...
            for user in users:
                ...
                await bot.send_message(
                    chat_id=user.telegram_id, text=clean_report, parse_mode="HTML"
                )
