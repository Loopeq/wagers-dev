...
async def insert_bets_points(bets: List[BetAddDTO], match_id: int):
    ...
    async with db_helper.session_factory() as session:
        ...
        stmt = (
            ...
            .order_by(Bet.match_id, Bet.type, Bet.period, Bet.key, Bet.version.desc())
            .distinct(Bet.match_id, Bet.type, Bet.period, Bet.key)
        )
...
async def insert_bets_coeffs(bets: List[BetAddDTO]):
    ...
    async with db_helper.session_factory() as session:
        ...
        stmt = (
            select(Bet)
            .where(Bet.match_id.in_(bet_ids))
            .order_by(Bet.match_id, Bet.type, Bet.key, Bet.period, Bet.version.desc())
        )
        ...
        for bet in bets:
            ...
            if existing_bet:
                ...
                minimum = min(
                    abs(existing_bet.home_cf - bet.home_cf),
                    abs(existing_bet.away_cf - bet.away_cf),
                )
...
async def get_event_bets(match_id: int):
    async with db_helper.session_factory() as session:
        stmt = (
            select(Bet)
            .filter(Bet.match_id == match_id, Bet.point.is_not(None))
            .order_by(Bet.version.desc())
        )
        result = await session.execute(stmt)
