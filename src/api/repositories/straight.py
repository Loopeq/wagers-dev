...
async def get_mapped_comparison(comparison: dict, match_id: int, child_id: int, sport_id: int):
    mapped_comparison = []
...
async def get_straight(match_id: int, child_id: int, session: AsyncSession):
    ...
    mapped_changes, unique_periods = get_mapped_changes(changes=changes, 
                                        match_id=match_id, 
                                        child_id=child_id, 
...
async def get_straight_full_history(match_id: int, child_id: int, session: AsyncSession):
    default_straight = await get_straight(match_id=match_id, child_id=child_id, session=session)
    ...
    for home_event in home_history: 
        ...
        event_straight = await get_straight(match_id=event_id, child_id=event_child_id, session=session)
    ...
    analyzed = await get_analyzed(content=full_history_dump, tool_call_query=tool_call_query)
    return analyzed
