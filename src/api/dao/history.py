...
from src.data.models import MatchMember, Match, MatchResult, Bet, BetChange
    ...
    async def get_match_history_by_team_name(team_name: str, current_match_id: int, league_id: int):
        async with (async_session_factory() as session):
            ...
            for row in result.fetchall():
                high = await ChangesOrm.get_highest_changes(match_id=row.id, session=session, diff=3)
                history.append({
                    'match_id': row.id,
